package org.mandar.analysis.recsys2014.scorers;
...
import org.mandar.analysis.recsys2014.models.TFIDFModel;
...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
            ...
            output.set(e.getKey(),iv.dot(userVector)/userVector.norm());
    ...
    private SparseVector makeUserVector(long user) {
        ...
        double meanbuff = 0;
        int num_of_ratings = userRatings.size();
        for (Rating r: userRatings) {
            ...
            if (p != null /*&& p.getValue() >= 3.5*/) {
                ...
                meanbuff += (p.getValue()/num_of_ratings);
                // TODO Get the item's vector and add it to the user's profile
                MutableSparseVector mutcopy = model.getItemVector(p.getItemId()).mutableCopy();
                mutcopy.multiply(p.getValue());
                profile.add(mutcopy);
            }
        }
